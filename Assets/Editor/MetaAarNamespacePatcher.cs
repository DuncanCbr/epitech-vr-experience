...
public class MetaAarNamespacePatcher : IPreprocessBuildWithReport
{
    ...
    private static readonly (string packagePrefix, string aarRelativePath, string newNamespace)[] Patches = new[]
    {
        ( "com.meta.xr.sdk.core", "Plugins/AndroidOpenXR/OVRPlugin.aar", "com.oculus.Integration.core" ),
        ( "com.meta.xr.sdk.interaction", "Runtime/Plugins/Android/InteractionSdk.aar", "com.oculus.Integration.interaction" ),
    };
    ...
    {
        var prop = typeof(EditorApplication).GetProperty("sevenZipPath",
            System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Static);
        if (prop != null) return (string)prop.GetValue(null);
    ...
    private static void PatchAar(string sevenZa, string packageCache, string packagePrefix, string aarRelativePath, string newNamespace)
    {
        string[] candidates = Directory.GetDirectories(packageCache, packagePrefix + "@*", SearchOption.TopDirectoryOnly);
        if (candidates.Length == 0)
        {
            UnityEngine.Debug.LogWarning($"{Tag} Package '{packagePrefix}' not found in PackageCache.");
            return;
        }
        ...
        try
        {
            ...
            {
                UnityEngine.Debug.LogWarning($"{Tag} No AndroidManifest.xml inside {Path.GetFileName(aarPath)}");
                return;
            }
            ...
            if (!changed)
            {
                UnityEngine.Debug.Log($"{Tag} {Path.GetFileName(aarPath)} already has namespace {newNamespace}, skipping.");
                return;
            }
    ...
    private static string PatchPackageAttribute(string xml, string newNamespace, out bool changed)
    {
        ...
        if (match.Success && match.Groups[2].Value != newNamespace)
        {
            changed = true;
            return regex.Replace(xml, $"$1{newNamespace}$3");
        }
        changed = false;
        return xml;
    }
    ...
    private static void Run7za(string sevenZa, string args)
    {
        ...
        if (proc.ExitCode != 0)
            throw new System.Exception($"{Tag} 7za failed (exit {proc.ExitCode}): {stderr}");
    }
}